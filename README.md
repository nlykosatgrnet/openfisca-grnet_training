...
These elements are described in different folders. All the modelling happens within the `openfisca_grnet_training` folder.
...
```sh
pip install openfisca-grnet_training
```
...
```sh
git clone git://github.com/nlykosatgrnet/openfisca-grnet_training.git.git
cd openfisca-grnet_training.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_grnet_training
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_grnet_training/situation_examples/couple.json \
